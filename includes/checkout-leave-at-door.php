...
function wlad_checkout_options() {
	...
	$leave_at_door      = __( 'Leave At Door', 'woocommerce_leave_at_door' );
	$leave_at_door_text = (string) apply_filters( 'leave_at_door_text', $leave_at_door );

	$instructions       = __( 'Instructions for delivery driver', 'woocommerce_leave_at_door' );
	...
	// 	$current_package = new WC_Shipping_Rate;

	// 	var_dump( wc_cart_totals_shipping_method_label( $package['rates'] ) );
	// }
	// var_dump( $all_shipping_methods['local_pickup'] );
	// if ( 'local_pickup:16' == $chosen_shipping ) {
	// 	echo '<tr class="additional-shipping-fields"><th>' . __( 'Leave At Door', 'conditional-woo-checkout-field-pro' ) . '</th><td>';

	// 	// Add a custom checkbox field
	// 	woocommerce_form_field( 'leave_at_door', array(
	// 		'type' => 'checkbox',
	// 		'class' => array( 'form-row-wide' ),
	// 	), '' );
